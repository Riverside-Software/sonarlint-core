      ...
      steps {
        ...
        script {
          withEnv(["MVN_HOME=${tool name: 'Maven 3', type: 'hudson.tasks.Maven$MavenInstallation'}", "JAVA_HOME=${tool name: 'Corretto 11', type: 'jdk'}"]) {
            sh "$MVN_HOME/bin/mvn -Dmaven.test.failure.ignore=true -Dmaven.source.skip -DskipDistWindows -DskipNoArch clean org.jacoco:jacoco-maven-plugin:prepare-agent deploy"
          }
        }
